...
func TestNPMFreshnessUsesApparentAge(t *testing.T) {
	...
	expiry := proxy.prepareNPMCacheResponse(response, testNPMRoute("https://registry.npmjs.org"), request.URL.String(), current.Add(-5*time.Second), current)
	if !expiry.Equal(current.Add(10*time.Second)) || response.Header.Get("Age") != "50" {
